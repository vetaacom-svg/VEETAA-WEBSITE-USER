...
const SplashCursor: React.FC<SplashCursorProps> = ({
  SIM_RESOLUTION = 64, // Optimized resolution for performance
  DYE_RESOLUTION = 512,
  CAPTURE_RESOLUTION = 256,
  DENSITY_DISSIPATION = 2.5, // Reduced for smoother, softer, longer-lasting flow ("plus souple")
  VELOCITY_DISSIPATION = 1.5, // Reduced for smoother flow
  PRESSURE = 0.1,
  PRESSURE_ITERATIONS = 12, // Keeps GPU overhead low
  CURL = 4, // Swirls slightly more elegantly
  SPLAT_RADIUS = 0.3, // Larger splats for smoother looks
  SPLAT_FORCE = 6000,
  SHADING = true,
  COLOR_UPDATE_SPEED = 10,
  BACK_COLOR = { r: 0.0, g: 0.0, b: 0.0 },
  TRANSPARENT = true,
  RAINBOW_MODE = false, // Set to false by default to focus on Gold color
  COLOR = '#EAB308' // Gold color by default
}) => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const animationFrameId = useRef<number | null>(null);

  // Store config in a mutable ref to prevent re-initializing WebGL on parent updates
  ...
  // Keep the configuration ref updated dynamically without restarting the WebGL loop
  useEffect(() => {
    configRef.current = {
      SIM_RESOLUTION,
      DYE_RESOLUTION,
  ...
  useEffect(() => {
    ...
    const pointers = [new (Pointer as any)()];
  ...
  }, []); // Empty dependency array ensures it initializes exactly once and NEVER resets during re-renders
